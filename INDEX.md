...
| 🔴 "G1 闭环只发生在容器/委托两族的 3 参注册上"（`F-REG2-002` 的原始范围） | **实测扩表（2026-09-22 第八刀，分族探针，已移除）**：字符串族 / 多引用族 6 子族 / `FBindingRegistry` / `FStructRegistry` / `FOptionalRegistry` **同样闭环**，且量级更大 —— 每域周期 **注册 3198 / 显式释放 14（0.44%）/ 卸载残留 3184**（容器族修后对照 382 / 180 / 246 ⇒ 残留 **12.9 倍**）；其中 **3005 条（94%）是 `IsNeedFree=true` 的自持堆块**（`FReturn` 的 `CopyValue` 堆拷贝、编组 `new std::decay_t<T>`、`FRegister*`/`TConstructorHelper` 的 `new`）⇒ **性质比容器族重一级：多一层纯原生内存泄漏**。**修法可行性同步切片**：非地址映射 **1807 条/周期可安全弱化**、地址映射 **1391 注册/1377 残留条不可照搬**（须先定"反查命中已回收包装"的语义）。⚠️ **仍**未改动任何代码。见 [`08-…/07`](08-专项审计/07-G1句柄释放责任归属（2026-09-22）.md) **§5.2** |
