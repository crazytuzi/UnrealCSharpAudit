...
> 数据来源（**唯一来源**）：`聚合统计脚本 -Detail`（机器解析 47 份报告正文）+ `09-问题清单/01-全局问题总表.md`（834 行机器生成总表）。
> 状态：**已完成**（现状）
...
|---|---|
| 基于 **807 条**带 `[F-*]` 编号的发现（并称 P0 = 55、P1 = 200、P2 = 309、P3 = 243） | 机器解析 **834 条**发现（P0 **26** / P1 **128** / P2 **267** / P3 **381** / 撤销 **32**） |
...
1. **凡统计级别/结论/可达性，一律复用 `聚合统计.ps1`，不要自写正则。** 该脚本自身经历过三次工具 bug 修正：① 用"整行含 `P0`"判级别 ⇒ P0 虚高到 **38**；② 未处理 `~~P2~~ → 撤销` 形态 ⇒ 残留 **27**；③ 用"整行含 `证伪`"判结论 ⇒ 证伪虚高到 **76**（实为 **26**）。
...
> ⚠️ **一处已知的口径不一致（如实记录）**：`09-问题清单/01-全局问题总表.md:22-27` 的表头仍写着"确认 478 / 部分确认 267 / 证伪 81 / 无法验证 8"，那是**未修正的"整行含 `证伪`"旧口径**；本文件与 `INDEX.md` 一律采用脚本输出的 **481 / 319 / 26 / 3（+5 非标准书写）**。

---

## §0 权威数字表（快照 T1-final，机器解析 47 份报告正文）

**取数方式**：`& "聚合统计.ps1" -Detail`
**取数时间**：`2026-09-11 01:05:23`｜**解析口径**：用 `^#{2,4}\s*\[?F-<前缀>-<数字>\]?` 定位每条发现的标题行，取该块内**首个** `**严重度**` 字段为最终级别；`复核结论` 取块内首个含该词的行；`可达性` **同时**取独立字段与复核结论行内嵌的"可达性 X"两处。

### 0.1 总量与级别

| 指标 | 数值 | 校验 |
|---|---|---|
| 报告份数（机器枚举 `.md`） | **47** | 其中 **41 份**含标题式发现块 |
...
**取数**：前 5 列（发现数、级别、复核结论、可达性）来自 `聚合统计.ps1 -Detail` 的"逐份明细"；**引文行号吻合率**来自各份的 `lines_match / lines_sampled`（**不是推算**）。
...
| `09-问题清单/01-全局问题总表.md` | 0 | — | — | — | 不适用（机器生成的派生视图） |
...
| 对照：**机器全量**引用一次命中率 | **96.48%（2661/2758）**，**行号越界 0 处** | 两条**完全独立**的方法（人工抽样回源码 vs 机器全量解析）差 **0.04** 个百分点 |
...
**来源**：`09-问题清单/01-全局问题总表.md` 的 32 行"级别 = 撤销"（机器可复现：`^\| F-… \| 撤销 \|` 命中 **32** 行）。
...
存在一个**会直接污染机器聚合**的字段级缺陷：某些条目"**标题 / 复核结论 / 级别变动三处都写『撤销（非缺陷）』，但 `严重度` 字段仍留着 `P3`**" —— 实证是 `08-…/04:1748-1755` 的 **`F-ABI-022`**（`:1751` 严重度 = `P3`、`:1752` 复核结论 = 证伪（撤销，非缺陷）、`:1755` 级别变动 = `P3 → 撤销（非缺陷）`）。
...
2. 🔴 **把 `严重度` 字段与"撤销"判定同步** —— 这是**唯一会直接污染机器聚合**的字段缺陷（实证 `08-…/04:1748-1755` 的 `F-ABI-022`）；不改，则 `P3 = 381` 与 `撤销 = 32` **至少有一个是错的**（见 §4.3）。
...
| 项 | 状态 |
|---|---|
| 本文件所有聚合数字的来源 | **仅** 机器全量解析 47 份报告正文的输出（2026-09-11 01:05:23）与 `09-问题清单/01-全局问题总表.md`（834 行机器生成）；**无一处人工推算** |
...
| 本文件的行为约束声明 | ✅ **只修改了这 1 个文件**（`00-总览/02-执行摘要与Top发现.md`）；**未修改** `INDEX.md`、`09-问题清单/**`、`00-总览/01`、`00-总览/03`、`08-专项审计/**`、任何模块报告与插件源码。所有数字均来自脚本输出或实际读到的文件，**无编造**。 |
